...
from igu_recovery.utils.input_helpers import (
    C_SUCCESS, C_RESET, parse_db_row_to_group, print_header
)
...
def main():
    print(f"{C_SUCCESS}Starting Enhanced Random Test Runner (Per-Product Randomization)...{C_RESET}")
    ...
    for idx, row in df.iterrows():
        ...
        for sc_name, sc_func in scenarios:
            try:
                ...
                print(f"Error processing {product_name} - {sc_name}: {e}")
    ...
    print(report_df[["Product Name", "Origin", "Route A Dist (km)"]].head())
